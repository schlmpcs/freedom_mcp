#!/usr/bin/env python3
"""Breakout Trade Planner (Freedom24) — Minervini-style plans from VCP output.

Project-local override of the global skill. Reads VCP screener JSON, applies a
strict Minervini Gate, calculates position sizes using worst-case entry prices,
and outputs actionable trade plans as **Freedom24 ``place_order`` proposals**
(pre_place and post_confirm modes).

DECISION-SUPPORT ONLY: every order this emits is a PROPOSAL with
``confirm=False``. Freedom24 has no native bracket/OCO order, so each plan is a
group of three separate proposals (entry + protective stop + take-profit). The
user reviews and runs ``place_order`` manually.
"""
...
def _build_actionable(
    ...
    """Build an actionable order with trade plan and Freedom24 order proposals."""
    sector = base["sector"]
    ...
    # Build entry condition and Freedom24 order proposals
    entry_cond = build_entry_condition(
        pivot=pivot,
        max_chase_pct=args.max_chase_pct,
    )

    pre_place = build_pre_place_proposal(
        symbol=base["symbol"],
        qty=sizing["shares"],
    ...
    post_confirm = build_post_confirm_proposal(
        symbol=base["symbol"],
        qty=sizing["shares"],
        worst_entry=worst_entry,
        stop_loss=stop_loss,
...
def generate_markdown(plans: dict) -> str:
    ...
    lines = [
        "# Breakout Trade Plan (Freedom24)",
        f"**Generated:** {plans['generated_at']}",
        f"**Account Size:** ${plans['parameters']['account_size']:,.0f} | "
        f"**Base Risk:** {plans['parameters']['base_risk_pct']}%",
        "",
        "> Decision-support only. Every order below is a PROPOSAL with "
        "`confirm=False`. Freedom24 has no native bracket/OCO order, so the "
        "stop-loss and take-profit are SEPARATE follow-on orders to place "
        "manually after the entry fills.",
        "",
        "## Summary",
    ...
    if plans["actionable_orders"]:
        ...
        for i, order in enumerate(plans["actionable_orders"], 1):
            tp = order["trade_plan"]
            entry = order["order_proposals"]["pre_place"]["proposals"][0]
            lines.extend(
                [
                    f"### {i}. {order['symbol']} ({entry['ticker']}) — "
                    f"{order.get('company_name', '')}",
                    f"**Rating:** {order['rating_band']} ({order['composite_score']}) | "
                    f"**State:** {order['execution_state']}",
                    "",
                    "| Parameter | Value |",
                    "|-----------|-------|",
                    f"| Entry ticker | {entry['ticker']} |",
                    f"| Signal Entry (stop_limit price) | ${tp['signal_entry']:.2f} |",
                    f"| Worst Entry (limit_hint) | ${tp['worst_entry']:.2f} |",
                    f"| Stop Loss (separate sell stop) | ${tp['stop_loss_price']:.2f} |",
                    f"| Risk (worst) | {tp['risk_pct_worst']:.1f}% |",
                    f"| Target ({tp['reward_risk_ratio']}R, separate sell limit) | "
                    f"${tp['target_price']:.2f} |",
                    f"| Shares | {tp['shares']} |",
                    f"| Position Value | ${tp['position_value']:,.2f} |",
                    f"| Risk $ | ${tp['risk_dollars']:,.2f} |",
                    "",
                    "Proposals (all `confirm=False`, run `place_order` manually): "
                    "entry stop_limit -> after fill: protective sell stop + sell-limit target.",
                    "",
                ]
            )
    ...
    if plans["revalidation"]:
        ...
        for r in plans["revalidation"]:
            lines.append(
                f"- **{r['symbol']}** ({r.get('ticker', '')}) — pivot ${r['pivot']:.2f}, "
    ...
    lines.append("\n---\n*Disclaimer: Not investment advice. Proposals only — you execute manually.*\n")
    return "\n".join(lines)


def main():
    parser = argparse.ArgumentParser(
        description="Generate Freedom24 breakout trade plans from VCP screener output"
    )
