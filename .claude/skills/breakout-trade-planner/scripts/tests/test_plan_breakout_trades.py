...
class TestGeneratePlans:
    ...
    def test_actionable_has_order_proposals(self):
        result = _make_vcp_result(score=85.0)
        ...
        assert "pre_place" in order["order_proposals"]
        assert "post_confirm" in order["order_proposals"]

        pre = order["order_proposals"]["pre_place"]
        # Freedom24: no native bracket -> three separate place_order proposals.
        assert pre["broker"] == "freedom24"
        assert pre["has_native_bracket"] is False
        pre_props = pre["proposals"]
        assert [p["proposal_type"] for p in pre_props] == ["entry", "stop_loss", "take_profit"]
        # entry is a stop_limit buy with a .US ticker; all confirm=False
        assert pre_props[0]["order_type"] == "stop_limit"
        assert pre_props[0]["action"] == "buy"
        assert pre_props[0]["ticker"].endswith(".US")
        for p in pre_props:
            assert p["tool"] == "mcp__freedom24__place_order"
            assert p["confirm"] is False

        post = order["order_proposals"]["post_confirm"]
        post_props = post["proposals"]
        # post-confirm entry is a plain limit (breakout already confirmed)
        assert post_props[0]["order_type"] == "limit"
        for p in post_props:
            assert p["confirm"] is False
